...
struct FCreateBrowserWindowSettings
{
	FCreateBrowserWindowSettings()
		...
		, Context()
		, AltRetryDomains()
        , bUseAcceleratedPaint(false)
		, bManageInPool(false)
		, bCreateForReady(false)
	{ }
...
public:
	...
	virtual int32 RegisterJSParam(){ return -1;}
	virtual bool UnRegisterJSParam(int32 ID) { return false; }
	virtual void AppendJSParam(int32 ID, FProperty* Property, const void* ValueAddress) {};
	virtual void ClearJSParams(int32 ID){};
	virtual void* GetJSParams(int32 ID){ return nullptr;};
};
