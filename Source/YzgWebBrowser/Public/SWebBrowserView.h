...
DECLARE_DELEGATE_TwoParams(FOnBrowserMessageDelegate,const FString&,const FString&);
...
public:
	...
	/** Default constructor. */
	YZGWEBBROWSER_API SWebBrowserView();
	...
	YZGWEBBROWSER_API void DelayCreateWebBrowser();

    TSharedPtr<IWebBrowserWindow> GetBrowserWindow() { return BrowserWindow;};
	/**
	...
	*/
	...
	YZGWEBBROWSER_API bool IsInitialized() const;

	YZGWEBBROWSER_API bool IsWebBrowserValid() const;
	...
	YZGWEBBROWSER_API void SetParentWindow(TSharedPtr<SWindow> Window);
	...
	YZGWEBBROWSER_API void SetBrowserKeyboardFocus();
	...
	YZGWEBBROWSER_API void CloseBrowser();

    YZGWEBBROWSER_API bool IsMouseTransparency();
...
private:
	...
	YZGWEBBROWSER_API void SetupParentWindowHandlers();

	/** Callback for document loading state changes. */
	YZGWEBBROWSER_API void HandleBrowserWindowDocumentStateChanged(EWebBrowserDocumentState NewState);

	/** Callback to tell slate we want to update the contents of the web view based on changes inside the view. */
	YZGWEBBROWSER_API void HandleBrowserWindowNeedsRedraw();

	/** Callback for document title changes. */
	YZGWEBBROWSER_API void HandleTitleChanged(FString NewTitle);

	/** Callback for loaded url changes. */
	YZGWEBBROWSER_API void HandleUrlChanged(FString NewUrl);

	/** Callback for showing browser tool tips. */
	YZGWEBBROWSER_API void HandleToolTip(FString ToolTipText);
	 ...
	 */
	YZGWEBBROWSER_API bool HandleBeforeNavigation(const FString& Url, const FWebNavigationRequest& Request);

	YZGWEBBROWSER_API bool HandleLoadUrl(const FString& Method, const FString& Url, FString& OutResponse);
	 ...
	 */
	YZGWEBBROWSER_API bool HandleCreateWindow(const TWeakPtr<IWebBrowserWindow>& NewBrowserWindow, const TWeakPtr<IWebBrowserPopupFeatures>& PopupFeatures);

    YZGWEBBROWSER_API void HandleBrowserMessage(const FString& MsgName,const FString& MsgParams);

    YZGWEBBROWSER_API bool HandleAttachBrowser();
	/**
	 ...
	 */
	YZGWEBBROWSER_API bool HandleCloseWindow(const TWeakPtr<IWebBrowserWindow>& BrowserWindow);

	/** Callback for showing dialogs to the user */
	YZGWEBBROWSER_API EWebBrowserDialogEventResponse HandleShowDialog(const TWeakPtr<IWebBrowserDialog>& DialogParams);

	/** Callback for dismissing any dialogs previously shown  */
	YZGWEBBROWSER_API void HandleDismissAllDialogs();

	/** Callback for popup window permission */
	YZGWEBBROWSER_API bool HandleBeforePopup(FString URL, FString Target);

	/** Callback for showing a popup menu */
	YZGWEBBROWSER_API void HandleShowPopup(const FIntRect& PopupSize);

	/** Callback for hiding the popup menu */
	YZGWEBBROWSER_API void HandleDismissPopup();

	/** Callback from the popup menu notifiying it has been dismissed */
	YZGWEBBROWSER_API void HandleMenuDismissed(TSharedRef<IMenu>);
	...
	YZGWEBBROWSER_API void HandleWindowDeactivated();
	YZGWEBBROWSER_API void HandleWindowActivated();

	YZGWEBBROWSER_API bool UnhandledKeyDown(const FKeyEvent& KeyEvent);
	YZGWEBBROWSER_API bool UnhandledKeyUp(const FKeyEvent& KeyEvent);
	YZGWEBBROWSER_API bool UnhandledKeyChar(const FCharacterEvent& CharacterEvent);

	YZGWEBBROWSER_API bool HandleDrag(const FPointerEvent& MouseEvent);
	YZGWEBBROWSER_API void HandleConsoleMessage(const FString& Message, const FString& Source, int32 Line, EWebBrowserConsoleLogSeverity Serverity);

	YZGWEBBROWSER_API TOptional<FSlateRenderTransform> GetPopupRenderTransform() const;
private:
...
protected:
	YZGWEBBROWSER_API bool HandleSuppressContextMenu();
private:
	TOptional<FCreateBrowserWindowSettings> BrowserSetting;
	float MouseTransparencyAlpha = 1.0f;
    bool bManageInPool = false;

    struct BindObjectData
    {
        FString Name;
        TWeakObjectPtr<UObject> Object;
        bool bIsPermanent;
    };
    TArray<BindObjectData> DelayBindObject;

	bool bAttachBrowser = false;
};
