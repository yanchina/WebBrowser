...
public:
	...
	*/
	...
	virtual void Refresh(bool bForce){};

	virtual FVector2D GetMouseLocalPosition() const { return FVector2D(); }
	/**
	...
	virtual int32 GetBrowserID() const { return -1;}

	virtual FString GetInitialURL() const { return FString(); };
	/**
	...
	virtual void ExecuteJSFunction(const FString& JSObj, int32 ParamsID) {};
	/**
	...
	*/
    ...
    virtual float GetWindowDPI() { return 1.0f; }; 
    virtual bool IsMouseTransparency(FIntPoint LocalMousePos) { return false;};
    virtual void SetResolution(const FIntPoint& InResolution) {};
	virtual TOptional<FIntPoint> GetResolution() const { return TOptional<FIntPoint>(); };
    virtual void SetBrowserTexture(UTexture* Texture){};
	virtual void MarkForDelete(float DelayDeleteTime = 2.f){};
	virtual bool IsMarkForDelete() { return false;};
	virtual bool IsReadyForDelete() { return true;};
	virtual bool IsManageInPool(){ return false; };
	virtual bool AttachBrowser(void* AttachDataPtr){ return false; };
	//virtual bool ReAttachBrowser(){return false;};
	virtual void Tick(float DeltaTime){};
public:
    /** Attach Browser */
    DECLARE_DELEGATE_RetVal(bool,FOnAttachBrowser);
    virtual FOnAttachBrowser& OnAttachBrowser() { static FOnAttachBrowser AttachBrowser;  check(0);  return AttachBrowser; };

    DECLARE_DELEGATE_TwoParams(FOnBrowserMessage, const FString&, const FString&);
    virtual FOnBrowserMessage& OnBrowserMessage() { static FOnBrowserMessage BrowserMessage;  check(0);  return BrowserMessage; };
